...
/**
 * Covers the HTTP Range-header parsing added to Y2WebServer for the embedded file manager's
 ...
 */
public class Y2WebServerRangeTest {
    ...
    public void noRangeHeader_servesWholeFileNonPartial() {
        Y2WebServer.RangeResult r = Y2WebServer.parseRange(null, FILE_LEN);
    ...
    public void headerNotStartingWithBytes_ignored() {
        Y2WebServer.RangeResult r = Y2WebServer.parseRange("frobnicate=0-99", FILE_LEN);
        assertFalse(r.partial);
        assertEquals(0, r.start);
        assertEquals(FILE_LEN - 1, r.end);
    }
    ...
    public void explicitStartEnd_returnsExactSlice() {
        ...
        Y2WebServer.RangeResult r = Y2WebServer.parseRange("bytes=0-999", FILE_LEN);
    ...
    public void suffixRange_returnsLastNBytes() {
        ...
        Y2WebServer.RangeResult r = Y2WebServer.parseRange("bytes=-500", FILE_LEN);
        assertTrue(r.partial);
    ...
    public void suffixRangeLargerThanFile_clampsToWholeFile() {
        Y2WebServer.RangeResult r = Y2WebServer.parseRange("bytes=-999999999", FILE_LEN);
        assertTrue(r.partial);
        assertEquals(0, r.start);
        assertEquals(FILE_LEN - 1, r.end);
    }
    ...
    public void openEndedRange_extendsToEndOfFile() {
        Y2WebServer.RangeResult r = Y2WebServer.parseRange("bytes=100-", FILE_LEN);
        assertTrue(r.partial);
    ...
    public void endBeyondFileLength_clampsToLastByte() {
        Y2WebServer.RangeResult r = Y2WebServer.parseRange("bytes=0-99999999999", FILE_LEN);
        assertTrue(r.partial);
        assertEquals(0, r.start);
        assertEquals(FILE_LEN - 1, r.end);
    }
    ...
    public void startGreaterThanEnd_notPartial() {
        ...
        Y2WebServer.RangeResult r = Y2WebServer.parseRange("bytes=500-100", FILE_LEN);
        assertFalse(r.partial);
    }

    @Test
    public void malformedNumber_fallsBackToWholeFile() {
        Y2WebServer.RangeResult r = Y2WebServer.parseRange("bytes=abc-def", FILE_LEN);
        assertFalse(r.partial);
        assertEquals(0, r.start);
        assertEquals(FILE_LEN - 1, r.end);
    }
    ...
    public void noDash_ignored() {
        Y2WebServer.RangeResult r = Y2WebServer.parseRange("bytes=12345", FILE_LEN);
        assertFalse(r.partial);
        assertEquals(0, r.start);
        assertEquals(FILE_LEN - 1, r.end);
    }
