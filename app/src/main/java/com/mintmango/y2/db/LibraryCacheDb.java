...
public class LibraryCacheDb extends SQLiteOpenHelper {
    ...
    public void saveNavidromeArtists(List<com.mintmango.y2.subsonic.SubsonicArtist> artists) {
        try {
            SQLiteDatabase db = getWritableDatabase();
            db.beginTransaction();
            try {
                ...
                int order = 0;
                for (com.mintmango.y2.subsonic.SubsonicArtist a : artists) {
                    ContentValues cv = new ContentValues();
                    cv.put("id", a.id);
                    cv.put("name", a.name);
    ...
    public List<com.mintmango.y2.subsonic.SubsonicArtist> loadNavidromeArtists() {
        List<com.mintmango.y2.subsonic.SubsonicArtist> result = new ArrayList<>();
        try {
            Cursor c = getReadableDatabase().query(ARTISTS_TABLE, null, null, null, null, null, "sort_order ASC");
            try {
                while (c.moveToNext()) {
                    com.mintmango.y2.subsonic.SubsonicArtist a = new com.mintmango.y2.subsonic.SubsonicArtist();
