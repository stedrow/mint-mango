...
import com.mintmango.y2.managers.AudioPlayerManager;
    ...
    private void acquireLocks(Context context) {
        try {
            ...
            if (wakeLock == null) {
                ...
                wakeLock = pm.newWakeLock(android.os.PowerManager.PARTIAL_WAKE_LOCK, "y2:Cast");
                wakeLock.setReferenceCounted(false);
            }
            if (!wakeLock.isHeld()) wakeLock.acquire();
            if (wifiLock == null) {
                android.net.wifi.WifiManager wm = (android.net.wifi.WifiManager) app.getSystemService(Context.WIFI_SERVICE);
                wifiLock = wm.createWifiLock(android.net.wifi.WifiManager.WIFI_MODE_FULL_HIGH_PERF, "y2:Cast");
    ...
    private TrackMeta extractLocalMeta(File track) {
        ...
        try {
            if (main != null && main.libraryCacheDb != null) {
                com.mintmango.y2.db.LibraryCacheDb.MetaOverride ov = main.libraryCacheDb.getMetaOverride(track.getAbsolutePath());
                if (ov != null) {
            ...
            @Override public void run() {
                try {
                    ...
                    if (meta.art != null && meta.art.length > 0) {
                        ...
                        a.ivAlbumArt.setImageResource(com.mintmango.y2.R.drawable.default_album);
                    }
                } catch (Exception e) {
