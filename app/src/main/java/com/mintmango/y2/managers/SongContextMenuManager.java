...
public class SongContextMenuManager {
    ...
    public void showQuickMenu(final MainActivity a) {
        ...
        final boolean isCasting = com.mintmango.y2.cast.CastManager.getInstance().isCasting();
        ...
        showThemedOptionsDialog(a, a.t("Quick Menu"), null,
                ...
                new Runnable[]{
                        ...
                        new Runnable() { @Override public void run() {
                            if (isCasting) {
                                com.mintmango.y2.cast.CastManager.getInstance().stopCasting();
    ...
    private void showCastMenu(final MainActivity a) {
        final com.mintmango.y2.cast.CastManager cast = com.mintmango.y2.cast.CastManager.getInstance();
        final java.util.List<com.mintmango.y2.cast.CastDevice> found = new ArrayList<>();

        Toast.makeText(a, a.t("Searching for Google speakers…"), Toast.LENGTH_SHORT).show();
        cast.startDiscovery(a, new com.mintmango.y2.cast.CastDiscovery.Callback() {
            @Override public void onDevicesChanged(java.util.List<com.mintmango.y2.cast.CastDevice> devices) {
                found.clear();
        ...
        h.postDelayed(new Runnable() {
            @Override public void run() {
                ...
                final java.util.List<com.mintmango.y2.cast.CastDevice> devices = new ArrayList<>(found);
                java.util.Collections.sort(devices, new java.util.Comparator<com.mintmango.y2.cast.CastDevice>() {
                    @Override public int compare(com.mintmango.y2.cast.CastDevice d1, com.mintmango.y2.cast.CastDevice d2) {
                ...
                for (int i = 0; i < devices.size(); i++) {
                    final com.mintmango.y2.cast.CastDevice dev = devices.get(i);
    ...
    private void showQueueDialog(final MainActivity a) {
        ...
        listView.setOnItemClickListener(new android.widget.AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(android.widget.AdapterView<?> parent, View view, int position, long id) {
                a.clickFeedback();
                dialog.dismiss();
                com.mintmango.y2.managers.AudioPlayerManager.getInstance().playTrackList(a.currentPlaylist, position);
            }
        });
