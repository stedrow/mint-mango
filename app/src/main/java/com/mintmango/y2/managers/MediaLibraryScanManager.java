...
public class MediaLibraryScanManager {
    ...
    private static final class ScanTask {
        ...
        final com.mintmango.y2.db.LibraryCacheDb.CachedSong cachedResult; // non-null = cache hit, no future
        final java.util.concurrent.Future<com.mintmango.y2.db.LibraryCacheDb.CachedSong> future; // non-null = cache miss

        ScanTask(File file, boolean isAudiobook, com.mintmango.y2.db.LibraryCacheDb.CachedSong cachedResult,
                 java.util.concurrent.Future<com.mintmango.y2.db.LibraryCacheDb.CachedSong> future) {
            this.file = file;
     ...
     *  one after another. */
    private com.mintmango.y2.db.LibraryCacheDb.CachedSong extractTags(MainActivity a, File f, long mtime, long size, boolean isAudiobook) {
        ...
        return new com.mintmango.y2.db.LibraryCacheDb.CachedSong(
    ...
    private void buildCustomLibrary(MainActivity a, File folder, List<ScanTask> pendingTasks,
                                     Map<String, com.mintmango.y2.db.LibraryCacheDb.CachedSong> cachedSongs,
                                     boolean isAudiobook,
        ...
        if (files != null) {
            for (final File f : files) {
                if (f.isDirectory()) {
                    ...
                    com.mintmango.y2.db.LibraryCacheDb.CachedSong cached = cachedSongs.get(f.getAbsolutePath());
                    ...
                    if (cached != null && cached.mtime == mtime && cached.size == size) {
                        ...
                        java.util.concurrent.Future<com.mintmango.y2.db.LibraryCacheDb.CachedSong> future =
                                scanExecutor.submit(new java.util.concurrent.Callable<com.mintmango.y2.db.LibraryCacheDb.CachedSong>() {
                                    @Override
                                    public com.mintmango.y2.db.LibraryCacheDb.CachedSong call() {
    ...
    private void finalizeScanTasks(final MainActivity a, List<ScanTask> pendingTasks, List<SongItem> newCustomLibrary,
                                    ...
                                    List<com.mintmango.y2.db.LibraryCacheDb.CachedSong> freshEntries) {
        for (ScanTask task : pendingTasks) {
            com.mintmango.y2.db.LibraryCacheDb.CachedSong result = task.cachedResult;
            if (result == null) {
                try {
    ...
    public void loadLibraryFromCacheInstant(MainActivity a) {
        Map<String, com.mintmango.y2.db.LibraryCacheDb.CachedSong> cachedSongs = a.libraryCacheDb.loadAll();
        if (cachedSongs.isEmpty()) return;

        MainActivity.customLibrary.clear();
        MainActivity.audiobookLibrary.clear();
        a.trackNumberMap.clear();
        for (com.mintmango.y2.db.LibraryCacheDb.CachedSong cached : cachedSongs.values()) {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    if (pm != null) {
                        scanLock = pm.newWakeLock(android.os.PowerManager.PARTIAL_WAKE_LOCK, "y2:LibraryScan");
                        scanLock.acquire();
                    }
                } catch (Exception e) {
                ...
                Map<String, com.mintmango.y2.db.LibraryCacheDb.CachedSong> cachedSongs = a.libraryCacheDb.loadAll();
                List<com.mintmango.y2.db.LibraryCacheDb.CachedSong> freshEntries = new ArrayList<>();
