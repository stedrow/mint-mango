...
public class NowPlayingUiManager {
    ...
    public void setupVisualizer(final MainActivity a) {
        try {
            ...
            a.audioVisualizer = new android.media.audiofx.Visualizer(com.mintmango.y2.managers.AudioPlayerManager.getInstance().getAudioSessionId());
    ...
    public void toggleFavorite(MainActivity a) {
        ...
        if (path == null) {
            if (com.mintmango.y2.managers.AudioPlayerManager.getInstance().isNavidromeMode) {
    ...
    public void adjustVolume(MainActivity a, boolean up) {
        ...
        if (com.mintmango.y2.cast.CastManager.getInstance().isCasting()) {
            ...
            com.mintmango.y2.cast.CastManager.getInstance().setVolume(level);
        }
        ...
        try {
            com.mintmango.y2.managers.FmRadioManager fm = com.mintmango.y2.managers.FmRadioManager.getInstance(a);
            if (fm.isPowerUp) {
                int streamFm = 10;
    ...
    public void commitScrub(MainActivity a) {
        ...
        com.mintmango.y2.managers.AudioPlayerManager.getInstance().seekAbsolute(target);
    }
    ...
    public boolean handleMediaSeekKeyRepeat(MainActivity a, KeyEvent event, int seekMs) {
        if (event.getRepeatCount() == 0) {
            ...
            if (now - a.lastSeekTime > 300) {
                ...
                a.lastSeekTime = now;
                com.mintmango.y2.managers.AudioPlayerManager.getInstance().seekRelative(seekMs);
                a.clickFeedback();
            }
        }
        return true;
