        ...
        while (shouldRun) {
            ...
            try {
                ...
                if (resetByPeer || sessionMs < SHORT_SESSION_MS) {
                    ...
                    com.mintmango.y2.managers.BluetoothAudioManager.getInstance().noteAapContention(targetMac);
                }
                try {
                    socket.close();
                } catch (Throwable t) {
    ...
    private void handleEarDetectionForAutoPause(AapState s, String source) {
        ...
        if (bothInEar && !nowBothInEar) {
            ...
            mainHandler.post(new Runnable() {
                @Override
                public void run() {
                    com.mintmango.y2.managers.AudioPlayerManager.getInstance().pauseForAirpods();
                }
            });
            ...
            mainHandler.post(new Runnable() {
                @Override
                public void run() {
                    com.mintmango.y2.managers.AudioPlayerManager.getInstance().resumeForAirpods();
                }
            });
        }
