...
public class Y2WebServer extends Thread {
    private static final String TAG = "Y2WebServer";
    ...
    private com.mintmango.y2.db.LibraryCacheDb libDb; // lazy; SQLite is process-shared so a fresh helper is fine
    public Y2WebServer(Context context) {
        this.context = context;
    }

    private synchronized com.mintmango.y2.db.LibraryCacheDb libraryDb() {
        if (libDb == null) {
            libDb = (com.mintmango.y2.MainActivity.instance != null && com.mintmango.y2.MainActivity.instance.libraryCacheDb != null)
                    ? com.mintmango.y2.MainActivity.instance.libraryCacheDb
                    : new com.mintmango.y2.db.LibraryCacheDb(context);
        }
        return libDb;
    }
    ...
    private static void appendAlbumJson(StringBuilder sb, com.mintmango.y2.subsonic.SubsonicAlbum al) {
    ...
    private static void appendSongJson(StringBuilder sb, com.mintmango.y2.subsonic.SubsonicSong s, boolean withDownloaded) {
     ...
     */
    private java.util.List<com.mintmango.y2.subsonic.SubsonicSong> resolveSongsFromBody(JSONObject obj) throws Exception {
        java.util.List<com.mintmango.y2.subsonic.SubsonicSong> songs = new java.util.ArrayList<>();
        String albumId = obj.optString("albumId", "");
        if (!albumId.isEmpty()) {
            java.util.List<com.mintmango.y2.subsonic.SubsonicSong> albumSongs =
                    com.mintmango.y2.subsonic.SubsonicClient.getInstance().getAlbumSongsBlocking(albumId);
            ...
            if (idFilter != null && idFilter.length() > 0) {
                ...
                for (com.mintmango.y2.subsonic.SubsonicSong s : albumSongs) if (keep.contains(s.id)) songs.add(s);
            } else {
            ...
            if (arr != null) for (int i = 0; i < arr.length(); i++) {
                songs.add(com.mintmango.y2.subsonic.SubsonicSong.fromWebJson(arr.getJSONObject(i)));
            }
        }
        return songs;
    }
    ...
    private void handleNavidromeDownload(OutputStream os, String body, boolean keepAlive) throws java.io.IOException {
        final java.util.List<com.mintmango.y2.subsonic.SubsonicSong> songs;
        ...
        final com.mintmango.y2.MainActivity a = com.mintmango.y2.MainActivity.instance;
        if (a == null) { sendJsonError(os, "Player app not running", keepAlive); return; }

        final boolean tr = transcoded;
        final boolean fr = force;
        final com.mintmango.y2.managers.NavidromeManager.EnqueueResult[] holder = new com.mintmango.y2.managers.NavidromeManager.EnqueueResult[1];
        final java.util.concurrent.CountDownLatch latch = new java.util.concurrent.CountDownLatch(1);
        a.runOnUiThread(new Runnable() { @Override public void run() {
            try { holder[0] = com.mintmango.y2.managers.NavidromeManager.getInstance().enqueueNavidromeDownloadsCore(a, songs, tr, fr); }
        ...
        com.mintmango.y2.managers.NavidromeManager.EnqueueResult r = holder[0];
    ...
    private void handleNavidromeDelete(OutputStream os, String body, boolean keepAlive) throws java.io.IOException {
        ...
        a.runOnUiThread(new Runnable() { @Override public void run() {
            try {
                com.mintmango.y2.managers.NavidromeManager nm = com.mintmango.y2.managers.NavidromeManager.getInstance();
                for (com.mintmango.y2.subsonic.SubsonicSong s : songs) if (nm.deleteNavidromeDownload(a, s)) deleted[0]++;
    ...
    private void handleLocalDelete(OutputStream os, String body, boolean keepAlive) throws java.io.IOException {
        ...
        final com.mintmango.y2.db.LibraryCacheDb db = libraryDb();
        int deleted = 0;
        ...
        if (a != null) {
            ...
            a.runOnUiThread(new Runnable() { @Override public void run() {
                try {
                    java.util.Iterator<com.mintmango.y2.models.SongItem> it = a.customLibrary.iterator();
    ...
    private class RequestHandler implements Runnable {
        ...
        private boolean handleRequest(InputStream is, OutputStream os, String requestLine) throws java.io.IOException {
            ...
            if (method.equals("GET") && path.startsWith("/api/local/albums")) {
                java.util.List<com.mintmango.y2.db.LibraryCacheDb.LocalAlbum> albums = libraryDb().loadLocalAlbums();
                StringBuilder json = new StringBuilder("{\"albums\":[");
                for (int i = 0; i < albums.size(); i++) {
                    com.mintmango.y2.db.LibraryCacheDb.LocalAlbum al = albums.get(i);
                    if (i > 0) json.append(",");
            ...
            if (method.equals("GET") && path.startsWith("/api/local/album")) {
                ...
                java.util.List<com.mintmango.y2.db.LibraryCacheDb.CachedSong> tracks = libraryDb().loadLocalAlbumTracks(artist, album);
                StringBuilder json = new StringBuilder("{\"tracks\":[");
                for (int i = 0; i < tracks.size(); i++) {
                    com.mintmango.y2.db.LibraryCacheDb.CachedSong s = tracks.get(i);
                    if (i > 0) json.append(",");
            ...
            if (method.equals("POST") && path.equals("/api/navidrome-settings")) {
                ...
                try {
                    ...
                    prefs.edit()
                            ...
                            .apply();
                    com.mintmango.y2.subsonic.SubsonicClient.getInstance().saveSettings(context, navUrl, navUser, navPass);
                    ...
                    com.mintmango.y2.subsonic.SubsonicClient.getInstance().ping(
                            new com.mintmango.y2.subsonic.SubsonicClient.Callback<Boolean>() {
            ...
            if (method.equals("GET") && path.startsWith("/api/navidrome/albums")) {
                if (!com.mintmango.y2.subsonic.SubsonicClient.getInstance().isConfigured()) { sendJsonError(os, "Navidrome is not configured — set the server URL and login on the Files page first.", keepAlive); return keepAlive; }
                java.util.Map<String, String> qp = parseQuery(path);
                ...
                try {
                    java.util.List<com.mintmango.y2.subsonic.SubsonicAlbum> albums =
                            com.mintmango.y2.subsonic.SubsonicClient.getInstance().getAlbumListBlocking(type, size, offset);
                    StringBuilder json = new StringBuilder("{\"albums\":[");
            ...
            if (method.equals("GET") && path.startsWith("/api/navidrome/search")) {
                ...
                try {
                    com.mintmango.y2.subsonic.SubsonicClient.SearchResult r =
                            com.mintmango.y2.subsonic.SubsonicClient.getInstance().searchBlocking(query, 30, 30);
                    StringBuilder json = new StringBuilder("{\"albums\":[");
            ...
            if (method.equals("GET") && path.startsWith("/api/navidrome/album")) {
                ...
                try {
                    java.util.List<com.mintmango.y2.subsonic.SubsonicSong> songs =
                            com.mintmango.y2.subsonic.SubsonicClient.getInstance().getAlbumSongsBlocking(id);
            ...
            if (method.equals("GET") && path.startsWith("/api/navidrome/cover")) {
                ...
                try {
                    ...
                    art = com.mintmango.y2.subsonic.SubsonicClient.getInstance().cacheCoverArtBlocking(id, size, cacheFile);
                } catch (Exception e) {
            ...
            if (method.equals("GET") && path.startsWith("/api/navidrome/queue")) {
                sendJson(os, com.mintmango.y2.managers.NavidromeManager.getInstance().getWebQueueJson(), keepAlive);
                return keepAlive;
            }
            ...
            if (method.equals("POST") && path.startsWith("/api/navidrome/cancel")) {
                ...
                final java.util.concurrent.CountDownLatch latch = new java.util.concurrent.CountDownLatch(1);
                a.runOnUiThread(new Runnable() { @Override public void run() {
                    try { removed[0] = com.mintmango.y2.managers.NavidromeManager.getInstance().clearPendingNavidromeDownloads(); }
                    finally { latch.countDown(); }
                }});
