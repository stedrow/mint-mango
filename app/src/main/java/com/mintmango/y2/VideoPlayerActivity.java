...
public class VideoPlayerActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        videoView.setOnPreparedListener(mp -> {
            if (Build.VERSION.SDK_INT >= 23) {
                try {
                    float speed = com.mintmango.y2.managers.AudioPlayerManager.getInstance().getCurrentSpeed();
                    if (speed != 1.0f) {
    ...
    private static final int SCRUB_STEP_MS =
            com.mintmango.y2.managers.NowPlayingUiManager.SCRUB_STEP_MS;
    ...
    private void applySpeakerMute() {
        ...
        try {
            boolean externalAudio =
                    com.mintmango.y2.managers.BluetoothAudioManager.getInstance().isAnyDeviceConnected();
